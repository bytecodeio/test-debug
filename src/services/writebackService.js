...
export const updateQuestionType = async (id,questionType,sdk) => {
    const asyncFunction = async (id,questionType,sdk) => {
        const slugResponse = await sdk
            .ok(
            sdk.create_sql_query({
                connection_name: connection,
                sql: `UPDATE ${scratch_schema}.chat_history SET questionType='${questionType}' WHERE id='${id}'`,
            })
            )
        const response = await sdk.ok(sdk.run_sql_query(slugResponse.slug, "json"));
    ...
    return asyncFunction(id,questionType,sdk);
}
